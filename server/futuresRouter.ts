...
const LAND_STATUS = ["owned", "leased", "under_negotiation", "planned_acquisition"] as const;

const FUTURES_STATUS = ["draft", "active", "partially_contracted", "fully_contracted", "expired", "cancelled"] as const;

const EOI_STATUS = ["pending", "under_review", "accepted", "declined", "expired", "withdrawn"] as const;

const DELIVERY_FREQUENCIES = ["monthly", "quarterly", "semi-annual", "annual", "flexible"] as const;

const PAYMENT_TERMS = ["net_30", "net_60", "net_90", "on_delivery", "advance", "negotiable"] as const;
...
export const futuresRouter = router({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!supplier) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'Only suppliers can create futures listings' });
      }

      // Validate projection years
      if (input.projectionEndYear < input.projectionStartYear) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'End year must be after start year' });
      }
      if (input.projectionEndYear - input.projectionStartYear > 25) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'Projection period cannot exceed 25 years' });
      }
      ...
      if (projections && projections.length > 0) {
        await db.bulkUpsertProjections(id, projections.map(p => ({
          ...p,
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const supplier = await db.getSupplierByUserId(ctx.user.id);
      if (!supplier) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'Only suppliers can update futures listings' });
      }

      const futures = await db.getFuturesById(input.id);
      if (!futures) {
        throw new TRPCError({ code: 'NOT_FOUND', message: 'Futures listing not found' });
      }
      if (futures.supplierId !== supplier.id) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'You can only update your own futures listings' });
      }
  ...
  list: protectedProcedure
    .query(async ({ ctx }) => {
      const supplier = await db.getSupplierByUserId(ctx.user.id);
      if (!supplier) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'Supplier profile required' });
      }
  ...
  publish: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (futures.supplierId !== supplier.id) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'You can only publish your own futures listings' });
      }

      // Check if there are projections
      const projections = await db.getProjectionsByFuturesId(input.id);
      if (projections.length === 0) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'Add yield projections before publishing' });
      }
  ...
  unpublish: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (futures.supplierId !== supplier.id) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'You can only unpublish your own futures listings' });
      }
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (futures.supplierId !== supplier.id) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'You can only delete your own futures listings' });
      }
      if (futures.status !== 'draft') {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'Only draft futures can be deleted' });
      }
  ...
  saveProjections: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (futures.supplierId !== supplier.id) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'You can only update projections for your own futures listings' });
      }

      await db.bulkUpsertProjections(input.futuresId, input.projections.map(p => ({
        ...p,
        projectedTonnes: p.projectedTonnes.toString(),
      })));
  ...
  submitEOI: protectedProcedure
    .input(z.object({
      ...
      annualVolumeTonnes: z.number().positive("Annual volume must be positive"),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const buyer = await db.getBuyerByUserId(ctx.user.id);
      if (!buyer) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'Only buyers can submit EOIs' });
      }
      ...
      if (futures.status !== 'active') {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'This futures listing is not accepting EOIs' });
      }

      // Check if buyer already has EOI for this futures
      const existingEOI = await db.getEOIByFuturesAndBuyer(input.futuresId, buyer.id);
      if (existingEOI) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'You have already submitted an EOI for this futures' });
      }
      ...
      if (input.interestStartYear < futures.projectionStartYear || input.interestEndYear > futures.projectionEndYear) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'Interest period must be within the futures projection period' });
      }
      ...
      if (totalVolumeTonnes > availableTonnes) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: `Requested volume exceeds available supply (${availableTonnes.toLocaleString()}t)` });
      }
  ...
  // Get buyer's EOIs
  myEOIs: protectedProcedure
    .query(async ({ ctx }) => {
      const buyer = await db.getBuyerByUserId(ctx.user.id);
      if (!buyer) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'Buyer profile required' });
      }
  ...
  getEOIsForFutures: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (futures.supplierId !== supplier.id) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'You can only view EOIs for your own futures listings' });
      }
  ...
  respondToEOI: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (futures.supplierId !== supplier.id) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'You can only respond to EOIs for your own futures listings' });
      }
  ...
  withdrawEOI: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (eoi.buyerId !== buyer.id) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'You can only withdraw your own EOIs' });
      }

      // Only pending or under_review EOIs can be withdrawn
      if (!['pending', 'under_review'].includes(eoi.status)) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'Only pending EOIs can be withdrawn' });
      }
