...
test.describe("Page Load & Status Tests", () => {
  for (const pageInfo of PUBLIC_PAGES) {
    test(`${pageInfo.name} - loads successfully`, async ({ page }) => {
      ...
      // Check HTTP status
      expect(response?.status(), `${pageInfo.name} should return 200`).toBe(
        200
      );
      ...
      expect(
        errorBoundary,
        `${pageInfo.name} should not show error boundary`
      ).toBe(0);
      ...
      expect(
        bodyText?.length,
        `${pageInfo.name} should have content`
      ).toBeGreaterThan(50);
    });
  }
});
...
test.describe("Mobile Responsiveness Tests", () => {
  test("Home page is mobile responsive", async ({ page }) => {
    ...
    const mobileMenu = page.locator(
      "[aria-label*='menu'], [aria-label*='Menu'], button:has(svg), .hamburger, [data-mobile-menu]"
    );
    ...
    const navVisible = await page
      ...
      .catch(() => false);
    expect(
      menuVisible || navVisible,
      "Mobile navigation should be accessible"
    ).toBe(true);
  });

  test("All public pages render on mobile without overflow", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    ...
    for (const pageInfo of PUBLIC_PAGES.slice(0, 5)) {
      ...
      expect(
        hasOverflow,
        `${pageInfo.name} should not have horizontal overflow on mobile`
      ).toBe(false);
    }
  });
});
...
test.describe("Link Checker - 404 Detection", () => {
  test("Home page links are valid", async ({ page }) => {
    ...
    const internalLinks = links.filter(
      link =>
        link.startsWith("/") && !link.includes("#") && !link.includes("mailto:")
    );
    ...
    for (const link of uniqueLinks) {
      const response = await page
        .goto(link, { waitUntil: "domcontentloaded", timeout: 10000 })
        .catch(() => null);
...
test.describe("Visual Consistency Tests", () => {
  test("Primary colors are consistent", async ({ page }) => {
    ...
    const greenElements = await page
      .locator("[class*='green'], [class*='primary'], .bg-primary")
      .count();
    expect(
      greenElements,
      "Should have primary colored elements"
    ).toBeGreaterThan(0);
  });
  ...
  test("Typography is consistent across pages", async ({ page }) => {
    ...
    if (validSizes.length > 1) {
      ...
      expect(unique.size, "H1 sizes should be consistent").toBeLessThanOrEqual(
        2
      );
    }
  });
  ...
  test("Buttons have consistent styling", async ({ page }) => {
    ...
    const buttons = await page
      .locator("button, a[class*='btn'], a[class*='button'], [role='button']")
      .all();

    if (buttons.length > 0) {
      const styles: string[] = [];
      for (const btn of buttons.slice(0, 5)) {
        const borderRadius = await btn
...
test.describe("Form Functionality Tests", () => {
  ...
  test("Financial onboarding form is accessible", async ({ page }) => {
    ...
    const formElements = await page
      .locator("form, [role='form'], .wizard, .steps, input, button")
      .count();
    expect(
      formElements,
      "Financial onboarding should have interactive elements"
    ).toBeGreaterThan(0);
  });
});
...
test.describe("Dashboard Access Tests", () => {
  for (const pageInfo of DASHBOARD_PAGES) {
    test(`${pageInfo.name} - handles auth gracefully`, async ({ page }) => {
      ...
      expect(
        response?.status(),
        `${pageInfo.name} should not return 500`
      ).toBeLessThan(500);
      ...
      expect(
        bodyText?.length,
        `${pageInfo.name} should have some content`
      ).toBeGreaterThan(20);
    });
  }
});
...
test.describe("Client Journey Flow Tests", () => {
  test("Grower journey: Home -> For Growers -> Registration", async ({
    page,
  }) => {
    // Start at home
    await page.goto("/", { waitUntil: "networkidle" });
    ...
    const growerLink = page
      .locator("a:has-text('Grower'), a:has-text('grower'), a[href*='grower']")
      .first();
    ...
    const registerCTA = page
      .locator(
        "a:has-text('Register'), a:has-text('Get Started'), a:has-text('Sign Up'), button:has-text('Register')"
      )
      .first();
    const ctaVisible = await registerCTA.isVisible().catch(() => false);
    expect(ctaVisible, "For Growers page should have registration CTA").toBe(
      true
    );
  });

  test("Developer journey: Home -> For Developers -> Browse", async ({
    page,
  }) => {
    ...
    expect(
      hasRelevantContent,
      "For Developers page should have relevant content"
    ).toBe(true);

    // Should have browse/marketplace CTA
    const browseCTA = page
      .locator(
        "a[href*='browse'], a:has-text('Browse'), a:has-text('Marketplace')"
      )
      .first();
  ...
  test("Lender journey: Home -> For Lenders -> Portal", async ({ page }) => {
    ...
    expect(
      hasRelevantContent,
      "For Lenders page should have relevant content"
    ).toBe(true);
  });
});
...
test.describe("SEO & Accessibility Basics", () => {
  test("All pages have title tags", async ({ page }) => {
    for (const pageInfo of PUBLIC_PAGES.slice(0, 5)) {
      ...
      expect(
        title.length,
        `${pageInfo.name} should have a title`
      ).toBeGreaterThan(0);
    }
  });
