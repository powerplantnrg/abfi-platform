...
async function collectDesignMetrics(page: Page, pagePath: string, pageName: string): Promise<DesignMetrics> {
  const errors: string[] = [];
  ...
  const primaryColors = await page.evaluate(() => {
    ...
    const primaryElements = document.querySelectorAll("[class*='primary'], [class*='emerald'], [class*='amber'], [class*='blue']");
...
test.describe("Design Consistency Tests", () => {
  ...
  test("Color palette - Primary colors are from brand palette", async ({ page }) => {
    const allColors: Set<string> = new Set();
    
    for (const pageConfig of PAGES.slice(0, 5)) { // Test first 5 pages
      await page.goto(pageConfig.path, { waitUntil: "networkidle" });
      
      const colors = await page.evaluate(() => {
        const colorSet = new Set<string>();
        const elements = document.querySelectorAll("button, [class*='bg-'], [class*='text-']");
        elements.forEach((el) => {
          const computed = window.getComputedStyle(el);
  ...
  test("Navigation - Consistent header/nav across pages", async ({ page }) => {
    const navigationConsistency: { page: string; hasNav: boolean; hasHeader: boolean }[] = [];
    ...
    // Log inconsistencies
    const withoutNav = navigationConsistency.filter((p) => !p.hasNav && !p.hasHeader);
    if (withoutNav.length > 0) {
      console.log("Pages without navigation:", withoutNav.map((p) => p.page));
    }
    
    // Most pages should have navigation
    const navCount = navigationConsistency.filter((p) => p.hasNav || p.hasHeader).length;
...
test.describe("Page-Specific Design Tests", () => {
  test("Home page - Hero section exists", async ({ page }) => {
    ...
    const ctaButtons = page.locator("a[href*='/for-'], a[href*='register'], button:has-text('Get Started')");
  ...
  test("For Growers page - Emerald theme colors", async ({ page }) => {
    ...
    const greenElements = await page.evaluate(() => {
      const elements = document.querySelectorAll("[class*='emerald'], [class*='green']");
  ...
  test("For Developers page - Amber theme colors", async ({ page }) => {
    ...
    const amberElements = await page.evaluate(() => {
      const elements = document.querySelectorAll("[class*='amber'], [class*='yellow'], [class*='orange']");
      return elements.length;
    });
  ...
  test("For Lenders page - Blue theme colors", async ({ page }) => {
    ...
    const blueElements = await page.evaluate(() => {
      const elements = document.querySelectorAll("[class*='blue'], [class*='indigo']");
      return elements.length;
    });
